...
info_artists <- artists %>%
  ...
  summarise(n = n(),
            ...
            nationality_other_percent = sum(artist_nationality_other == "Other")/n(),
            race_nwi_percent = sum(artist_race_nwi == "Non-White")/n(),
            race_wi_percent = sum(artist_race_nwi == "White")/n())

# Spacing
info_space <- artists %>%
  summarise(space_mean = mean(space_ratio_per_page_total),
...
# caption 
caption <- paste0("In total Gardner's Art Through the Ages and Jansons's History of Art feature ",
                 info_artists$n, " artists. ", info_artists$n_nk, " are not knwon by name.",
                 " The majority of artists are males (", 
                 scales::percent(info_artists$male_percent),"),<br>just ", 
                 scales::percent(info_artists$female_percent), " are female and the gender of ",
                 scales::percent(info_artists$gender_nk_percent), " is not known.",
                 ...
                 scales::percent(info_artists$race_wi_percent), " are white.",
                 "In the art history textbooks each artist where presented on",
                 scales::percent(info_space$space_mean), " of a page. The most attention was given to ",
                 info_space_artists$artist_name[1], "<br>in the ", info_space_artists$edition_number[1], 
                 "th edition of ", info_space_artists$book[1], " with ", round(info_space_artists$space_ratio_per_page_total[1], 1), 
                 " pages. Where as the shortest paragraph was written for ",
                 info_space_artists$artist_name[2], " with ",
                 scales::percent(info_space_artists$space_ratio_per_page_total[2], accuracy = 0.1),
                 " of a page in the ", info_space_artists$edition_number[2], " edition of ",
                 info_space_artists$book[2], ".<br><br>",
